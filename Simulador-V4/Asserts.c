...
char *elements[2][38]={{
	...
	"Relative MEMory Operand 2",
	"Absolute MEMory OPeration code",
	"Absolute MEMory Operand 1",
	"Absolute MEMory Operand 2",
	"Program Counter",
	"ACCumulator",
	"IR OPeration code",
	"IR Operand 1",
	...
	"MMU - MAR",
	"Main MEMory MAR",
	"Main Memory MBR OPeration code",
	"Main Memory MRB Operand 1",
	"Main Memory MBR Operand 2",
	"eXecuting PID",
	"Relative MEMory",
	"Absolute MEMory",
	...
	"Main Memory MBR",
	"Process Table - state",
	"Process Table - copyOfPCRegister",
	"Process Table - priority",
	"Process Table - copyOfSPRegister",
...
int elementNumber(char *cmp) {
 ...
 while ((elements[0][n]!=NULL) && strcmp(cmp, elements[0][n]))
   n++;

 if (elements[0][n]==NULL) return -1;
 else return n;
...
//  assertsQueue = (heapItem *) malloc (MAX_ASSERTS*sizeof(heapItem));
 ...
 ComputerSystem_DebugMessage(NO_TIMED_MESSAGE,81, POWERON, ASSERTS_FILE, MAX_ASSERTS);
   ...
   while (fgets(lineRead,MAXLINELENGTH, mf) != NULL && numberAsserts<MAX_ASSERTS) {
	...
	if (strcmp(time,"*")) {
	  ...
	  if (rc==0){
		...
		ComputerSystem_DebugMessage(NO_TIMED_MESSAGE,85,POWERON,lineNumber,ASSERTS_FILE);
		continue;
	  }
	}
   ...
   if (numberAsserts==MAX_ASSERTS)
		...
		ComputerSystem_DebugMessage(NO_TIMED_MESSAGE,83,POWERON,lineNumber);
   else
...
void assertMsg(int time, int ele, int expectedValue, int realValue, int addr) {
	int en;

	if (GEN_ASSERTS) { 
		genAssertMsg(time, elements[0][ele], realValue,addr);
		return;  // only generate, not checking
	}	

	// printf("Assert failed. Time: %d; Element: %s; ", time, ele);
	ComputerSystem_DebugMessage(NO_TIMED_MESSAGE,88,ERROR, time, elements[1][ele], elements[0][ele]);
	en=elementNumber(elements[0][ele]);
	...
	switch (op) {
		case RMEM_OP: 
				...
				if ((operationCode!=asserts[na].value) || GEN_ASSERTS)
					assertMsg(globalCounter,RMEM_OP,asserts[na].value,operationCode,asserts[na].address);
				break;
		case RMEM_O1:  	
				busData.cell=mainMemory[MMU_GetBase()+asserts[na].address];
				operand1=Processor_DecodeOperand1(busData);
				if ((operand1!=asserts[na].value) || GEN_ASSERTS)
					assertMsg(globalCounter,RMEM_O1,asserts[na].value,operand1,asserts[na].address);
				break;		
		case RMEM_O2:
				busData.cell=mainMemory[MMU_GetBase()+asserts[na].address];
				operand2=Processor_DecodeOperand2(busData);
				if ((operand2!=asserts[na].value) || GEN_ASSERTS)
					assertMsg(globalCounter,RMEM_O2,asserts[na].value,operand2,asserts[na].address);
			  	break;
		case AMEM_OP: 				       
				busData.cell=mainMemory[MMU_GetBase()+asserts[na].address];
				operationCode=Processor_DecodeOperationCode(busData);
				if ((operationCode!=asserts[na].value) || GEN_ASSERTS)																																			
					assertMsg(globalCounter,AMEM_OP,asserts[na].value,operationCode,asserts[na].address);
				break;
		case AMEM_O1:  
				busData.cell=mainMemory[asserts[na].address];
				operand1=Processor_DecodeOperand1(busData);
				if ((operand1!=asserts[na].value)|| GEN_ASSERTS)
					assertMsg(globalCounter,AMEM_O1,asserts[na].value,operand1,asserts[na].address);
			 	break;
		case AMEM_O2:
				busData.cell=mainMemory[asserts[na].address];
				operand2=Processor_DecodeOperand2(busData);
				if ((operand2!=asserts[na].value)|| GEN_ASSERTS)
					assertMsg(globalCounter,AMEM_O2,asserts[na].value,operand2,asserts[na].address);
			 	break;	 		 	
		case PC:  
		...
		case IR_OP: 
				...
				if ((operationCode!=asserts[na].value)|| GEN_ASSERTS)
					assertMsg(globalCounter,IR_OP,asserts[na].value,operationCode,0);
				break;
		case IR_O1:
				operand1=Processor_DecodeOperand1(registerIR_CPU);
				if ((operand1!= asserts[na].value)|| GEN_ASSERTS)
					assertMsg(globalCounter,IR_O1,asserts[na].value,operand1,0);
				break;
		case IR_O2:
				operand2=Processor_DecodeOperand2(registerIR_CPU);
				if ((operand2!=asserts[na].value)|| GEN_ASSERTS)
					assertMsg(globalCounter,IR_O2,asserts[na].value,operand2,0);			
				break;
		case PSW:
		...
		case MBR_OP:
				...
				if ((operationCode!=asserts[na].value)|| GEN_ASSERTS)
					assertMsg(globalCounter,MBR_OP,asserts[na].value,operationCode,0);
				break;
		case MBR_O1:
				operand1=Processor_DecodeOperand1(registerMBR_CPU);
				if ((operand1!= asserts[na].value)|| GEN_ASSERTS)
					assertMsg(globalCounter,MBR_O1,asserts[na].value,operand1,0);
				break;
		case MBR_O2:
				operand2=Processor_DecodeOperand2(registerMBR_CPU);
				if ((operand2!=asserts[na].value) || GEN_ASSERTS)
					assertMsg(globalCounter,MBR_O2,asserts[na].value,operand2,0);	
				break;
		case MMU_BS:
		...
		case MMBR_OP:
				...
				operationCode=Processor_DecodeOperationCode(busData);
				if ((operationCode!=asserts[na].value) || GEN_ASSERTS)
					assertMsg(globalCounter,MMBR_OP,asserts[na].value,operationCode,0);
				break;
		case MMBR_O1:
				MainMemory_GetMBR(&data);
				busData.cell=data;
				operand1=Processor_DecodeOperand1(busData);
				if ((operand1!=asserts[na].value) || GEN_ASSERTS)
					assertMsg(globalCounter,MMBR_O1,asserts[na].value,operand1,0);
				break;
		case MMBR_O2:
				MainMemory_GetMBR(&data);
				busData.cell=data;
				operand2=Processor_DecodeOperand2(busData);
				if ((operand2!=asserts[na].value) || GEN_ASSERTS)
					assertMsg(globalCounter,MMBR_O2,asserts[na].value,operand2,0);
				break;
		case XPID:
		...
		case MBR:
			if ((registerMBR_CPU.cell != asserts[na].value)|| GEN_ASSERTS)
					assertMsg(globalCounter,MBR,asserts[na].value,registerMBR_CPU.cell,0);
				break;
		case MMBR:
				MainMemory_GetMBR(&data);
				busData.cell=data;
				if ((busData.cell != asserts[na].value)|| GEN_ASSERTS)
					assertMsg(globalCounter,MMBR,asserts[na].value,busData.cell,0);
				break;
		case PCB_ST:
		...
		case SP:  
				if ((registerSP_CPU!=asserts[na].value)|| GEN_ASSERTS)
					assertMsg(globalCounter,SP,asserts[na].value,registerSP_CPU,0);
				break;
		case RA:  
				if ((registerA_CPU!=asserts[na].value)|| GEN_ASSERTS)
					assertMsg(globalCounter,RA,asserts[na].value,registerA_CPU,0);
				break;
		case RB:  
