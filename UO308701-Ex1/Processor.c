...
void Processor_InitializeInterruptVectorTable(int interruptVectorInitialAddress) {
	...
	interruptVectorTable[MODEEXCEPCION_BIT] = interruptVectorInitialAddress + NUMBER_OF_CPU_INSTRUCTIONS_PER_INTERRUPT;
...
void Processor_DecodeAndExecuteInstruction() {
	...
	switch (operationCode) {
		...
		case DIV_INST: 
			if (operand2 == 0)
				Processor_RaiseInterrupt(MODEEXCEPCION_BIT); 
			else {
				registerAccumulator_CPU=operand1 / operand2;
				registerPC_CPU++;
			}
